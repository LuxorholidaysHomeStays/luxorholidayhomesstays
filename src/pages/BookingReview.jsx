...
export default function BookingReview() {
  ...
  const handleBack = () => {
    ...
    // Clear the booking review data since we're going back to the booking form
    localStorage.removeItem('pendingBookingReview');
    navigate(`/villa/${bookingData.villaId}`, { 
      state: { 
        bookingData: editedData,
        fromReview: true 
      } 
    });
  };
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (!calculatedTotalAmount) {
        const nights = Math.max(1, Math.ceil((new Date(editedData.checkOutDate).getTime() - new Date(editedData.checkInDate).getTime()) / (1000 * 3600 * 24)));
        let totalPrice = 0;
      ...
      const orderResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'https://luxorstay-backend.vercel.app'}/api/payments/create-order`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const nights = Math.ceil((new Date(editedData?.checkOutDate).getTime() - new Date(editedData?.checkInDate).getTime()) / (1000 * 3600 * 24));
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[600px]">
            ...
            <div className="relative overflow-hidden">
              <img 
                src={getVillaImage(editedData?.villaName)} 
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  {editMode ? (
                    ...
                    <div className="text-sm text-gray-700">
                      {editedData?.address?.street}, {editedData?.address?.city}, {editedData?.address?.state}, {editedData?.address?.country} {editedData?.address?.zipCode}
                    </div>
                  )}
                </div>
